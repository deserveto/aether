...
export function process(schema, ctx, _params = { path: [], schemaPath: [] }) {
    ...
    if (overrideSchema) {
        ...
        if (schema._zod.processJSONSchema) {
            ...
            // [Aether patch] Fall back to the default processor map when the
            // caller's ctx.processors omits this type. Mastra dev's schema walk
            // builds a context without the default map, causing wrapper types
            // (optional/default/nullable/...) to crash as "Non-representable".
            const processor = ctx.processors[def.type] ?? allProcessors[def.type];
            if (!processor) {
